...
// Only create client if env vars are present to avoid crashing on import
...
export default async function handler(req, res) {
  ...
  // Set agent as connected (service role bypasses RLS — always works)
  ...
  // Agent's turn with new move
  if(game.turn==='b'&&game.move_count>lastMoveCount){
    let chess;
    try{chess=new Chess(game.fen);}
    catch(e){return res.status(500).json({error:'Corrupt game state',code:'CORRUPT_FEN'});}
