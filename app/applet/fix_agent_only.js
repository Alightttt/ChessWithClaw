...
agentCode = agentCode.replace(
  /<div className="flex justify-between items-center bg-neutral-900 p-4 rounded-lg">/g,
  `<div className="flex justify-between items-center bg-neutral-900 p-4 rounded-lg" style={{ display: 'none' }}>`
);
agentCode = agentCode.replace(
  /<div className="flex-1 overflow-y-auto max-h-40 flex flex-col gap-2">/g,
...
// Find the last </div>
const matchIndex = agentCode.lastIndexOf('</div>');
agentCode = agentCode.substring(0, matchIndex) + `      <div data-testid="current-fen" style={{ display: 'none' }}>{game.fen || 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1'}</div>
    ...
    </div>` + agentCode.substring(matchIndex + 6);
