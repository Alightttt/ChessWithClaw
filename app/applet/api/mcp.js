...
// chess.js resolves as an ES Module in this function's dependency trace —
// require() can never load it (confirmed via production logs: ERR_REQUIRE_ESM
// on every single request). Node's own fix for this is a dynamic import(),
// which loads both CJS and ESM. Cached after first load so it only pays the
// import cost once per cold start, not once per tool call.
...
function buildServer() {
    ...
    'join_game',
    {
      ...
      inputSchema: {
        ...
        webhook_url: z.string().optional().describe(
          'Optional. A URL of yours ChessWithClaw can call when something happens in this game — your turn, your human said something, a draw was offered. Point it at wherever you actually want to be reached. If you skip this, nothing pushes to you — you decide when to check in instead.'
        ),
      },
    },
    async ({ invite_code, webhook_url }) => {
      ...
      await supabase
        ...
        .update({
          ...
          agent_webhook_url: webhook_url || null,
        })
        .eq('id', game.id);

      // Investment stage: surface the ongoing relationship, not just this game.
      const { data: bond } = await supabase
        .from('bonds')
        .select('*')
        .eq('agent_token', game.agent_token)
        .eq('human_id', game.human_id || game.id)
        .maybeSingle();
      ...
      return toolText({
        ...
        history_with_this_human: bond
          ? {
              games_played: bond.games_played,
              record: `${bond.wins}W-${bond.losses}L-${bond.draws}D`,
              notable_moments: bond.notable_moments || [],
            }
          : { games_played: 0, record: 'first game', notable_moments: [] },
  ...
  server.registerTool(
    ...
    async ({ game_id, agent_token, move, thought, chat }) => {
      ...
      if (isGameOver) {
        const humanId = game.human_id || game.id;
        const { data: existing } = await supabase
          ...
          .eq('human_id', humanId)
          .maybeSingle();
        const wonByAgent = winner === 'black'; // agent always plays black in this app
        const wonByHuman = winner === 'white';
        await supabase.from('bonds').upsert({
          agent_token,
          human_id: humanId,
          games_played: (existing?.games_played || 0) + 1,
          wins: (existing?.wins || 0) + (wonByAgent ? 1 : 0),
          losses: (existing?.losses || 0) + (wonByHuman ? 1 : 0),
          draws: (existing?.draws || 0) + (!wonByAgent && !wonByHuman ? 1 : 0),
          notable_moments: [
            ...(existing?.notable_moments || []),
            ...(moveHistory.length > 60
              ? [{ type: 'long_game', moves: moveHistory.length, game_id, ts: new Date().toISOString() }]
              : []),
          ].slice(-10), // keep the 10 most recent, not an unbounded list
          updated_at: new Date().toISOString(),
        }, { onConflict: 'agent_token,human_id' });
      }

      // Turn just passed to the human — let the agent know without it polling,
      // if it gave us somewhere to tell it.
      if (!isGameOver && game.agent_webhook_url) {
        fetch(game.agent_webhook_url, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ event: 'human_turn_pending_after_your_move', game_id }),
        }).catch(() => {}); // fire-and-forget — a dead webhook must never break the move itself
      }
