...
export default function Game() {
  ...
  useEffect(() => {
    ...
    // Subscribe to dedicated chats table
  ...
  const getAgentMood = useCallback(() => {
    ...
    const agentAdvantage = playerColor === 'w' ? -balance : balance;
    const isAgentTurn = currentTurn === (playerColor === 'w' ? 'b' : 'w');
    if (isCheckState && isAgentTurn) return 'stressed';
  ...
  const getMaterialBalance = (fen) => {
    ...
    let balance = 0;
    for (const ch of board) { if (vals[ch]) { if (ch === ch.toUpperCase()) balance += vals[ch]; else balance -= vals[ch]; } }
    return balance;
  };

  const handleIllegalMove = useCallback(() => { setShaking(true); setTimeout(() => setShaking(false), 500); }, []);
  const handleCapture = useCallback((piece) => { playSound('capture'); }, [playSound]);
    ...
    <div ref={containerRef} className="relative text-white font-sans selection:bg-red-500/30 transition-colors duration-700 box-border scrollbar-none" style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: (game?.turn !== (game?.player_color || 'w') && game?.status === 'active') ? 'radial-gradient(ellipse at 50% 0%, rgba(230,57,70,0.07) 0%, transparent 70%)' : 'transparent', transition: 'background 0.8s ease', position: 'relative' }}>
        ...
        <Modal title="Settings" onClose={() => setShowSettings(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '10px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div style={{ fontSize: '14px', fontWeight: 600, color: '#999' }}>Board Theme</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
                {['green', 'blue', 'brown', 'purple', 'wood', 'dark'].map(t => (
                  <button key={t} onClick={() => { setBoardTheme(t); localStorage.setItem('cwc_board_theme', t); }} style={{ padding: '8px', borderRadius: '6px', border: boardTheme === t ? '2px solid #e63946' : '1px solid #333', background: t === 'dark' ? '#111' : t, textTransform: 'capitalize', fontSize: '12px' }}>{t}</button>
                ))}
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div style={{ fontSize: '14px', fontWeight: 600, color: '#999' }}>Piece Style</div>
              <select value={pieceStyle} onChange={(e) => { setPieceStyle(e.target.value); localStorage.setItem('cwc_piece_style', e.target.value); }} style={{ width: '100%', background: '#111', border: '1px solid #333', borderRadius: '6px', padding: '8px', color: '#fff' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '10px' }}>
              <button onClick={handleResign} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: 'none', background: '#e63946', color: '#fff', fontWeight: 700 }}>{confirmResign ? 'Confirm Resign?' : 'Resign Game'}</button>
              <button onClick={handleDraw} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #333', background: 'transparent', color: '#fff', fontWeight: 700 }}>{confirmDraw ? 'Confirm Offer?' : 'Offer Draw'}</button>
            </div>
          </div>
        </Modal>
      )}
      {showGameOver && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#111', padding: '40px', borderRadius: '16px', textAlign: 'center', border: '1px solid #222', maxWidth: '90%', width: '400px' }}>
            <div style={{ fontSize: '48px', marginBottom: '16px' }}>{game?.result === 'draw' ? '🤝' : '🏆'}</div>
            <div style={{ fontSize: '24px', fontWeight: 800, marginBottom: '8px', textTransform: 'uppercase' }}>Game Over</div>
            <div style={{ fontSize: '16px', color: '#999', marginBottom: '32px' }}>{game?.result === 'draw' ? 'The game ended in a draw' : `${game?.result} wins by ${game?.result_reason || 'checkmate'}`}</div>
            <div style={{ display: 'flex', gap: '12px' }}>
              <button onClick={handleRematch} className="design-btn-primary" style={{ flex: 1, padding: '14px', borderRadius: '10px', fontWeight: 700 }}>New Game</button>
              <button onClick={handleShareResult} style={{ padding: '14px', borderRadius: '10px', background: '#222', border: 'none', color: '#fff' }}><Share2 size={20} /></button>
            </div>
          </div>
        </div>
      )}
      {showLeaveWarning && (
        <Modal title="Quit Game?" onClose={() => setShowLeaveWarning(false)}>
          <div style={{ padding: '10px' }}>
            <p style={{ color: '#999', marginBottom: '24px', lineHeight: 1.5 }}>Are you sure you want to leave? Your OpenClaw will win by abandonment.</p>
            <div style={{ display: 'flex', gap: '12px' }}>
              <button onClick={() => setShowLeaveWarning(false)} style={{ flex: 1, padding: '12px', borderRadius: '8px', background: '#222', border: 'none', color: '#fff' }}>Stay</button>
              <button onClick={() => navigate('/')} style={{ flex: 1, padding: '12px', borderRadius: '8px', background: '#e63946', border: 'none', color: '#fff', fontWeight: 700 }}>Quit</button>
            </div>
          </div>
        </Modal>
      )}
