  ...
  const handleRealtimeUpdate = useCallback((payload) => {
    ...
    setGame(prev => {
      ...
      if (Array.isArray(newData.chat_history)) {
        setChatMessages((newData?.chat_history || []).slice(-50));
      }
      ...
      {isDesktop ? (
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto', padding: '12px', gap: '12px' }}>
          ...
          <div style={{ 
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '15px', fontWeight: 700 }}>{agentName}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                <div style={{ ...dotStyle, width: 6, height: 6 }} />
                <span style={{ fontSize: '10px', color: '#777', textTransform: 'uppercase' }}>{statusLabel}</span>
              </div>
            </div>
            {thoughtDisplay.visible && (
              <div style={{ fontSize: '12px', fontStyle: 'italic', color: 'rgba(255,255,255,0.5)', maxWidth: '40%' }}>
                "{thoughtDisplay.text}"
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: '#111', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          {/* MOBILE CHAT AREA */}
          <div style={{ flex: 1, minHeight: '300px', display: 'flex', flexDirection: 'column', background: '#0e0e0e', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', overflow: 'hidden' }}>
            <div ref={chatMessagesRef} style={{ flex: 1, overflowY: 'auto', padding: '12px' }}>
              {renderChatMessages()}
            </div>
            <form onSubmit={sendMessage} style={{ padding: '8px', background: '#111', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
              <div style={{ position: 'relative', display: 'flex' }}>
                <input 
                  type="text" value={chatInput} onChange={handleChatInputChange} placeholder="Chat..."
                  style={{ flex: 1, background: '#000', border: '1px solid #222', borderRadius: '8px', padding: '10px 12px', fontSize: '13px', color: '#fff' }}
                />
                <button type="submit" style={{ position: 'absolute', right: '4px', top: '4px', bottom: '4px', width: '36px', background: '#e63946', borderRadius: '6px', border: 'none', color: '#fff' }}>
                  <Send size={14} />
                </button>
              </div>
            </form>
